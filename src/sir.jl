...
# SIR (generator) where generator can be a compositional or a simple genreator...

################################
# Generic likelihood-weighting #
################################
#
# generate!(sir_generator, (num_samples, target_args, constraints), trace)
#
# trace is an AtomicTrace that stores a value that is a trace of target.
# trace is expected to have constraints matching those of the given constraints at all times.
# trace is expected to have constraints for the latent variables at all times.
...
# constraints is a trace of target with some choices constrained (represents the observations)
#
# NOTE: passing in the result of compose(target, proposal, mapping) as the generator is perfectly valid!

struct ResimulationSIRGenerator{T} <: AtomicGenerator{T}
    # target trace type is T
    target::Generator{T}
    latent_addresses::Set
end

function generate!(g::ResimulationSIRGenerator{T}, args::Tuple{Int, Tuple, T}, trace::AtomicTrace{T}) where {T}
    ...
    if (trace.mode == propose) || (trace.mode == record)
        ...
        # p(x_k, constraints), which is needed for the score
        for addr in g.latent_addresses
            constrain!(traces[chosen], addr, traces[chosen][addr])
        end
        (chosen_joint_score, _) = generate!(g.target, target_args, traces[chosen])
    ...
    elseif trace.mode == constrain
        ...
        # TODO problem: the score below will include the prior... it will not be hte likelihood...
